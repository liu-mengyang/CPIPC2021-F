...
void print_table()
{
	ofstream outfile("E://math_data//m3B_b.txt", ios::out);
	...
	ofstream outfile2("E://math_data//m3B_a.txt", ios::out);
	...
	ofstream outfile3("E://math_data//m3B_c.txt", ios::out);
...
void cal_ans()
{
	...
	for (int i = 0; i < jz2.size(); i++)
	{
		...
		for (int j = 0; j < rw2.size(); j++)
		{
			...
			renwu rw = rw2[j];
			//if (rw.beg_time - x.end >= 2880)
			if (cal_date(rw.beg_time) - cal_date(x.end) >= 3)
			{
	...
	//遍历p_base1找到其中替补次数
	int num = 0;
	for (int i = 0; i < p_base1.size(); i++)
	{
		partner pp = p_base1[i];
		people a = pp.a;
		if (a.cur_pos == 'F' && a.type == 3) num++;
		people b = pp.b;
		if (b.cur_pos == 'F' && b.type == 3) num++;
	}
	cout << "替补资格次数为：" << num << endl;

	//计算环成本：
	//统计每种任务环的个数
	int cir1_num = 0, cir2_num = 0, cir3_num = 0, cir4_num = 0;
	long long cir_cost = 0;
	for (auto x : p_base1)
	{
		vector<renwu>rws = x.rws;
		for (auto y : rws)
		{
			cir_cost += (y.arr_time - y.beg_time)*20*2; //每小时20，一组两个人
			if (y.dutys.size() == 1) cir1_num++;
			else if (y.dutys.size() == 2) cir2_num++;
			else if (y.dutys.size() == 3) cir3_num++;
			else if (y.dutys.size() == 4) cir4_num++;
		}
	}
	for (auto x : p_base2)
	{
	...
	cout << "环成本为：" << cir_cost << "(记得除60)" << endl;
	cout << "任务环数量分布" << endl;
	cout << cir1_num << " " << cir2_num << " " << cir3_num << " " << cir4_num << endl;
...
void delete_renwu()
{
	...
	{
		renwu rw = renwus[i];
		if (rw.dutys.size() <= 4 && (rw.arr_time - rw.beg_time) <= max_cir)
		{
